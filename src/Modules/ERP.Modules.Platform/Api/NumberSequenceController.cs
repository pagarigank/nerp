...
public class NumberSequenceController : ControllerBase
{
    ...
    public NumberSequenceController(IUnitOfWork unitOfWork, IAuditLogService auditLogService)
    {
    ...
    {
        var sequences = await _unitOfWork.NumberSequences.FindAsync(x => x.CompanyId == companyId, cancellationToken);
