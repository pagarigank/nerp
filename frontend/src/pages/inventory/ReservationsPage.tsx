...
export function ReservationsPage() {
  ...
  const { data: rows = [], isLoading } = useQuery({ queryKey: ['inventory', 'reservations'], queryFn: () => getReservations() })
  ...
  const createMut = useMutation({
    mutationFn: (d: Form) => createReservation({ companyId: companyId(), itemId: d.itemId, warehouseId: d.warehouseId, quantity: d.quantity, unitOfMeasure: d.unitOfMeasure, sourceType: d.sourceType, referenceId: d.referenceId || null, notes: d.notes || null }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['inventory', 'reservations'] }); setShowCreate(false); reset() },
    onError: e => setErr(getErrorMessage(e)),
  })
  const release = useMutation({ mutationFn: (id: string) => releaseReservation(id), onSuccess: () => qc.invalidateQueries({ queryKey: ['inventory', 'reservations'] }), onError: e => setErr(getErrorMessage(e)) })
  const cancel = useMutation({ mutationFn: (id: string) => cancelReservation(id), onSuccess: () => qc.invalidateQueries({ queryKey: ['inventory', 'reservations'] }), onError: e => setErr(getErrorMessage(e)) })
    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="New Reservation" size="lg"
        ...
        <form className="space-y-4" noValidate>
          ...
          <Textarea {...register('notes')} label="Notes" placeholder="Reservation notes..." rows={2} />
        </form>
      </Modal>

      <Card>
        <CardHeader title="Item Reservations" description={`${rows.length} reservation(s)`}
          action={<Button variant="primary" size="sm" onClick={() => { setErr(null); reset(); setShowCreate(true) }} leftIcon={<Plus className="h-4 w-4" />}>New Reservation</Button>} />
        <CardContent>
          {isLoading ? <p className="text-sm text-gray-500 py-8 text-center">Loading...</p> :
            rows.length === 0 ? <p className="text-sm text-gray-500 py-8 text-center">No reservations.</p> :
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="border-b border-gray-200 dark:border-gray-700 text-left">
                    ...
                    <th className="px-3 py-2 font-medium text-gray-500">UOM</th>
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-700/60">
                    {rows.map((r: ReservationSummary) => (
                      <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        ...
                        <td className="px-3 py-3"><Badge variant={r.status === 'Active' ? 'success' : 'neutral'} size="sm" dot>{r.status}</Badge></td>
                        <td className="px-3 py-3 text-right">
                          <div className="flex justify-end gap-1">
                            {r.status === 'Active' && <Button size="sm" variant="outline" disabled={release.isPending} onClick={() => release.mutate(r.id)}>Release</Button>}
                            {r.status === 'Active' && <Button size="sm" variant="ghost" className="text-red-600" disabled={cancel.isPending} onClick={() => cancel.mutate(r.id)}>Cancel</Button>}
                          </div>
                        </td>
                      </tr>
                    ))}
