...
import { z } from 'zod'
import { Plus, Search, AlertCircle, ChevronDown, ChevronRight, Eye, Edit3, Check, Trash2 } from 'lucide-react'
...
import {
  getPhysicalCounts, getPhysicalCount, createPhysicalCount,
  startPhysicalCount, completePhysicalCount, postPhysicalCount, cancelPhysicalCount,
  updatePhysicalCountLine, generateCountSheet,
  getWarehouses, getItems,
} from '@api/inventory'
import type {
  PhysicalCountSummary, PhysicalCountLineDto, CreatePhysicalCountRequest,
  PhysicalCountPostResult, WarehouseSummary, ItemSummary, CountSheet,
...
export function PhysicalCountsPage() {
  ...
  const [postResult, setPostResult] = useState<PhysicalCountPostResult | null>(null)
  ...
  const { data: rows = [], isLoading } = useQuery({
    queryKey: ['inventory', 'physical-counts'],
    queryFn: () => getPhysicalCounts(),
  })

  // Detail
  const { data: detail } = useQuery({
    queryKey: ['inventory', 'physical-count', viewId],
    queryFn: () => getPhysicalCount(viewId!),
    enabled: !!viewId,
  })

  // Mutations
  const start = useMutation({
    mutationFn: (id: string) => startPhysicalCount(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['inventory', 'physical-counts'] }); qc.invalidateQueries({ queryKey: ['inventory', 'physical-count'] }) },
    onError: e => setErr(getErrorMessage(e)),
  })
  const complete = useMutation({
    mutationFn: (id: string) => completePhysicalCount(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['inventory', 'physical-counts'] }); qc.invalidateQueries({ queryKey: ['inventory', 'physical-count'] }) },
    onError: e => setErr(getErrorMessage(e)),
  })
  const post = useMutation({
    mutationFn: (id: string) => postPhysicalCount(id),
    onSuccess: (result) => { qc.invalidateQueries({ queryKey: ['inventory', 'physical-counts'] }); qc.invalidateQueries({ queryKey: ['inventory', 'physical-count'] }); setPostResult(result) },
    onError: e => setErr(getErrorMessage(e)),
  })
  const cancel = useMutation({
    mutationFn: (id: string) => cancelPhysicalCount(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['inventory', 'physical-counts'] }); qc.invalidateQueries({ queryKey: ['inventory', 'physical-count'] }) },
    onError: e => setErr(getErrorMessage(e)),
  })

  // Create form
  const { register, handleSubmit, reset, watch, formState: { errors: createErrors } } = useForm<CreateForm>({
    resolver: zodResolver(createSchema),
    defaultValues: { warehouseId: '', countNumber: `PC-${new Date().toISOString().slice(0, 10).replace(/-/g, '')}`, countDate: new Date().toISOString().slice(0, 10), blindCount: false, notes: '', useCountSheet: true, abcClass: '' },
  })

  const createMutation = useMutation({
    mutationFn: (data: CreateForm) => {
      const body: CreatePhysicalCountRequest = {
      ...
      return createPhysicalCount(body)
    },
    onSuccess: (id) => { qc.invalidateQueries({ queryKey: ['inventory', 'physical-counts'] }); setShowCreate(false); reset(); setViewId(id) },
    onError: e => setErr(getErrorMessage(e)),
  })
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return rows
    return rows.filter((r: PhysicalCountSummary) => r.countNumber.toLowerCase().includes(q))
  }, [rows, search])
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Create Form Modal */}
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="New Physical Count" size="lg"
        ...
        <form className="space-y-4" noValidate>
          <div className="grid grid-cols-2 gap-4">
            ...
            <Input {...register('abcClass')} label="ABC Class Filter" placeholder="A, B, C, or leave empty for all" />
          </div>
          <Checkbox {...register('blindCount')} label="Blind Count" description="Hide system quantities from counters" />
          <Checkbox {...register('useCountSheet')} label="Auto-generate count sheet from warehouse stock" description="Pre-populate lines with all items in the selected warehouse" />
          <Textarea {...register('notes')} label="Notes" placeholder="Optional notes..." rows={2} />
        </form>
      </Modal>

      {/* Count Detail / Entry Modal */}
      <Modal isOpen={!!viewId} onClose={() => setViewId(null)} title={detail ? `Physical Count: ${detail.countNumber}` : 'Loading...'} size="xl"
      ...
      {/* List */}
      <Card>
        <CardHeader title="Physical Counts" description={`${rows.length} count(s)`}
        ...
        <CardContent>
          ...
          {isLoading ? <p className="text-sm text-gray-500 py-8 text-center">Loading...</p> :
            filtered.length === 0 ? <p className="text-sm text-gray-500 py-8 text-center">{search ? 'No matches.' : 'No physical counts yet.'}</p> :
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-700/60">
                    {filtered.map((r: PhysicalCountSummary) => (
                      <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 cursor-pointer" onClick={() => setViewId(r.id)}>
                        ...
                        <td className="px-3 py-3 text-gray-700 dark:text-gray-300">{r.blindCount ? 'Yes' : 'No'}</td>
...
function CountDetailView({ detail, onStart, onComplete, onPost, onCancel, isPending, isBlind }: {
  detail: PhysicalCountSummary
  onStart: () => void
  onComplete: () => void
  onPost: () => void
  onCancel: () => void
  ...
  const updateLine = useMutation({
    mutationFn: ({ lineId, qty, notes }: { lineId: string; qty: number; notes: string }) =>
      updatePhysicalCountLine(detail.id, lineId, { countedQuantity: qty, notes: notes || null }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['inventory', 'physical-count', detail.id] }); setEditingLine(null); setLineErr(null) },
    onError: e => setLineErr(getErrorMessage(e)),
  })

  const lines = detail.lines ?? []
  const varianceLines = lines.filter(l => l.variance != null && Math.abs(l.variance) > 0.0001)
  const missingCount = lines.filter(l => l.countedQuantity == null).length

  function startEdit(line: PhysicalCountLineDto) {
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead className="bg-gray-50 dark:bg-gray-800">
            <tr>
              ...
              <th className="px-3 py-2 text-left text-xs font-medium uppercase text-gray-500">Bin</th>
              {!isBlind && <th className="px-3 py-2 text-right text-xs font-medium uppercase text-gray-500">System Qty</th>}
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {lines.map(line => (
              <tr key={line.id} className="bg-white dark:bg-gray-900">
                ...
                <td className="px-3 py-2 text-sm text-gray-500">{line.binId ? line.binId.slice(0, 8) : '—'}</td>
                {!isBlind && <td className="px-3 py-2 text-sm text-right tabular-nums text-gray-700 dark:text-gray-300">{line.systemQuantity}</td>}
